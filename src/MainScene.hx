...
class MainScene extends Scene {
	...
	function startLevel(level:Int) {
		...
		if (level == 1)
			...
			background.texture = assets.texture(Images.MAP__MAP_2_GREEN_PLANES);
		else if (level == 3)
			background.texture = assets.texture(Images.MAP__MAP_3_ORANGE_FIELDS);
		else if (level == 4)
			background.texture = assets.texture(Images.MAP__MAP_4_ORANGE_BLUE_LAKE);
		else if (level == 5)
			background.texture = assets.texture(Images.MAP__MAP_5_BLUE_RIVER);
		else if (level == 6)
			background.texture = assets.texture(Images.MAP__MAP_6_BLUE_GREEN_ISLE);
		else if (level == 7)
			background.texture = assets.texture(Images.MAP__MAP_7_BLUE_GREEN_BEACH);
		...
		if (started) {
			...
			if (player.score >= scorelimit) {
				if (nextlevelbuttonactive == false) {
					...
					nextlevelbutton.x = 1500;
					nextlevelbutton.y = 1100;
